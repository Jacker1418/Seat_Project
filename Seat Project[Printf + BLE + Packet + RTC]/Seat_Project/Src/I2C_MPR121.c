...
static volatile uint8_t arrSensor_Buffer[BUFFER_SIZE];
static volatile uint16_t touchState;

static volatile uint8_t randIndex = 0;

uint16_t dummySensorValue[10][COUNT_KEY] = { {100, 70, 50,
																																				80,  40,  20,
																																				70,  50,  40},
																																			{23, 67, 11,
																																				45,  87,  64,
...
SensorValue_Status extractiSensorValue(volatile uint16_t *arrSensorValue){
	...
	*/
	...
	for(index = 0; index < COUNT_KEY; index++){
		arrSensorValue[index] = dummySensorValue[randIndex][index];
	}
