...
struct Touch_Data{
	volatile uint16_t touchState;
	volatile uint16_t sensorValue[9];
...
HAL_StatusTypeDef MPR121_Receive(volatile uint8_t* pData, uint8_t reg, uint32_t length, uint32_t Timeout);
