...
struct point position[COUNT_KEY] = {
																												{ -1, 1 },
																												{ 0, 1 },
																												{ 1, 1 },
																												{ -1, 0 },
...
int arrWeight[COUNT_KEY] = { 10, // -1, 1
																							 7,  // 0, 1
																							 10, // 1, 1
																							 8,  // -1, 0
																							 3,	 // 0, 0
...
SensorValue_Status getPosition(struct Touch_Data* data){
		...
		for (int i = 0; i < 9; i++) {
			sumOfX += data->sensorValue[i] * ((position[i].x * arrWeight[i]) + OFFSET_X);
			sumOfY += data->sensorValue[i] * ((position[i].y * arrWeight[i]) + OFFSET_Y);

			sumOfTotalMass += data->sensorValue[i];
		}

		data->position.x = (float)sumOfX / (float)sumOfTotalMass;
		data->position.y = (float)sumOfY / (float)sumOfTotalMass;
